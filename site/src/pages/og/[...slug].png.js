...
export async function getStaticPaths() {
  ...
  const ranked = pages.filter((page) => page.eligible)
  const total = ranked.length
  const top10 = ranked.slice(0, 10).map((p) => ({ name: p.name, score: p.score }))
  const slim = (p) => ({ name: p.name, eligible: p.eligible, rank: p.rank, total: p.total, score: p.score, coverage: p.coverage, signals: p.signals, usage: p.usage, momentum: p.momentum, stars: p.stars, usagePct: p.usagePct, momPct: p.momPct, attnPct: p.attnPct, kinds: p.kinds })
  const paths = []
  for (const locale of LOCALES) {
    ...
    pages.forEach((p) => {
      const shapes = p.eligible && p.rank <= V_TOP ? OG_SHAPES : ['h']
